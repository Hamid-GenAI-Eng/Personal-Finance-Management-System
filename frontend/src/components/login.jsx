...
const Login = () => {
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:5001/auth/login", formdata); // ✅ Fixed API URL
      alert(res.data.message);
      localStorage.setItem("isAuthenticated", "true"); // Store auth status
      navigate("/dashboard");
    } catch (err) {
      alert("Login failed: " + (err.response?.data?.message || err.message)); // ✅ Improved error handling
    }
  };
